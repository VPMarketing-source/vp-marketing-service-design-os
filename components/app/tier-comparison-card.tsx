    ...
    <Card className="h-full overflow-hidden p-0">
      <div className="border-b border-[var(--line)] px-5 py-4" style={{ backgroundColor: accent }}>
        <div className="flex items-center justify-between gap-4">
          ...
          <Badge tone="accent">Tier View</Badge>
        </div>
      ...
      <div className="space-y-5 p-5 text-sm">
        ...
        <div>
          ...
          <p className="mt-2 leading-6">{metric}</p>
        </div>
        <div>
          <p className="text-xs uppercase tracking-[0.16em] text-[var(--muted)]">Scope</p>
        ...
        <div>
          <p className="text-xs uppercase tracking-[0.16em] text-[var(--muted)]">QA Standard</p>
          <ul className="mt-2 space-y-2 text-[var(--foreground)]/88">
            {qa.map((item) => (
              <li key={item}>• {item}</li>
            ))}
